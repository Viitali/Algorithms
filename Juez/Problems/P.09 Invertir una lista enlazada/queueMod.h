...
public:
   ...
   void invierte() //funcion que pide el problema
   {
	   if (nelems == 0) //caso especial
		   return;

	   Nodo *nuevoUlt = prim;
	   Nodo *nuevoPrim = ult;
	   Nodo *ant = nullptr, *act = prim, *next = prim->sig;

	   while (next != nullptr) // le da la vuelta a la cola
	   {
		   act->sig = ant;
		   ant = act;
		   act = next;
   ...
   void print(std::ostream& o = std::cout) const { //para mostrar con el cout
	   Nodo *aux = prim;
	   while (aux != ult) {
		   o << aux->elem << ' ';
		   aux = aux->sig;
...
inline std::ostream & operator<<(std::ostream & salida, queue<int> const& lista) { //sobrecarga para mostrar
	lista.print(salida);
	return salida;
}
#endif // queue_eda_h
