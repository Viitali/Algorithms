...
void resolver(const vector<tObj> &d, int k, int &smax, int sloc, vector<bool> &solmej, vector<bool> &solloc, const int &presu, int presloc, const vector<int> &acum) {
	solloc[k] = true;
	if (esValida(d,k,presu,presloc)) //añadimos objeto
	{
		...
		{
			if (sloc + acum[k+1] > smax) //Estimacion para la etapa siguiente
				resolver(d, k + 1, smax, sloc, solmej, solloc,presu,presloc,acum);
		}
		sloc -= d[k].tam;
		presloc -= d[k].coste;
	}
...
bool resuelveCaso() {
	...
	for (int i = acumulado.size() - 1; i > 0; i--)
		acumulado[i - 1] += acumulado[i];
	/*Fin estimacion*/

	resolver(d,k,smax,sloc,solmej,solloc,presu,presloc,acumulado);
